...
if [[ "${BUILD_TYPE}" == "AppImage" ]]; then
    ...
    docker exec -d -v `pwd`:`pwd` -w `pwd` build-container -c "source ci/scripts/create_deb.sh"
    mv ksnip_*.deb ksnip-$VERSION-amd64.deb
elif [[ "${BUILD_TYPE}" == "rpm" ]]; then
    docker exec -d -v `pwd`:`pwd` -w `pwd` build-container -c "source ci/scripts/create_rpm.sh"
