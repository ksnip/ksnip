...
if [[ "${BUILD_TYPE}" == "AppImage" ]]; then
    ...
    nmake
    cd ..
    mkdir ksnip-$VERSION-x86_64
    cd ksnip-$VERSION-x86_64
    mv ../build/src/ksnip*.exe ksnip.exe
    windeployqt.exe ksnip.exe
    cd ..
    zip ksnip-$VERSION-windows-x86_64.zip ksnip-$VERSION-x86_64
fi
